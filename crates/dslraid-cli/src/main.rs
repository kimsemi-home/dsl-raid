    ...
    match cli.command {
        ...
        Command::Artifact { command } => match command {
            ArtifactCommand::Verify {
                ...
                lock,
                format,
            } => artifact_verify(&input, lock.as_deref(), format),
        },
...
fn quality() -> Result<()> {
    ...
    artifact_verify(input, None, OutputFormat::Text)?;
    println!("quality ok");
    Ok(())
}
...
fn artifact_verify(input: &Path, lock: Option<&Path>, format: OutputFormat) -> Result<()> {
